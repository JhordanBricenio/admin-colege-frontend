...
@Component({
  ...
  styleUrl: './students-index.component.css'
})
export class StudentsIndexComponent {
  ...
  private loadUsers(page: number) {
    this.studentService.getUsersByPageable(page).subscribe({
      next: (response) => {
        ...
        this.users = response.content as Student[];
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.studentService.deleteUser(id).subscribe(
          {
            next: () => {
              this.users = this.users.filter(user => user.idStudent !== id);
              Swal.fire({
                title: "¡Eliminado!",
